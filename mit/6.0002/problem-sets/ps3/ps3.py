...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        for x in range(width):
            for y in range(height):
                self.tiles[(x, y)] = int(dirt_amount)
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if self.tiles[tile] < capacity:
            ...
            self.tiles[tile] = self.tiles[tile] - capacity
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        return self.tiles[(int(m), int(n))] == 0
    ...
    def is_position_in_room(self, pos):
        ...
        cur_x, cur_y = pos.get_x(), pos.get_y()
        
        if cur_x >= 0 and cur_x < self.width:
            valid_x = True
        
        if cur_y >= 0 and cur_y < self.height:
            valid_y = True
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        #> Direction can be one of 360 degrees (float)
        self.direction = float(random.randint(0, 359))
        
        #> Position has random x, y coordinates (floats)
        x_cor = float(random.uniform(0, room.width))
        y_cor = float(random.uniform(0, room.height))
        self.position = Position(x_cor, y_cor)
...
class EmptyRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        Returns: an integer; the total number of tiles in the room
        """
        tile_coords = self.tiles.keys()
        num_tiles = int(len(tile_coords))
    ...
    def get_random_position(self):
        ...
        """
        x_cor = float(random.uniform(0, self.width))
        y_cor = float(random.uniform(0, self.height))
        
        return Position(x_cor, y_cor)
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        is_furnished = self.is_position_furnished(pos)
        
        return is_in_room and not is_furnished
    ...
    def get_num_tiles(self):
        ...
        tile_coords = self.tiles.keys()
        num_total_tiles = int(len(tile_coords))
        
        #> Number of tiles with furniture
        furniture_tile_coords = self.furniture_tiles
        num_furniture_tiles = int(len(furniture_tile_coords))
        
        return num_total_tiles - num_furniture_tiles
        
    def get_random_position(self):
        """
        Returns: a Position object; a valid random position (inside the room and not in a furnished area).
        """
        #> Keep going until a valid random position is generated
        while True:
            x_cor = float(random.uniform(0, self.width))
            y_cor = float(random.uniform(0, self.height))
            pos = Position(x_cor, y_cor)
            
            #> If random position has no furniture, it can be returned as valid
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        #> Get current position and direction to calculate a possible new position
        pos_current = self.get_robot_position()
        dir_current = self.get_robot_direction()
        pos_updated = pos_current.get_new_position(dir_current, self.speed)
        
        #> If new position is valid, move the robot to that position and clean the tile
        if self.room.is_position_valid(pos_updated):
            self.set_robot_position(pos_updated)
            self.room.clean_tile_at_position(pos_updated, self.capacity)
            
        #> Otherwise, set robot to new random direction
        else:
            dir_updated = float(random.randint(0, 359))
            self.set_robot_direction(dir_updated)
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        #> Determine if robot is faulty in current step
        gets_faulty = self.gets_faulty()
        ...
        #> If new position is valid and robot is not fauly, move the robot to that position and clean the tile
        if not gets_faulty and self.room.is_position_valid(pos_updated):
            self.set_robot_position(pos_updated)
            self.room.clean_tile_at_position(pos_updated, self.capacity)
...
# Uncomment this line to see your implementation of FaulyRobot in action!
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for _ in range(num_trials):
        ...
        #> Details about the room to be cleaned
        room = EmptyRoom(width, height, dirt_amount)
        num_tiles_total = room.get_num_tiles()
        num_tiles_clean = room.get_num_cleaned_tiles()
        
        #> Details about the robots
        robots = []
        ...
        #> Keep cleaning until some minimum percentage of tiles are cleaned
        while True:
            #> If minimum coverage is met, break out of the loop
            if float(num_tiles_clean / num_tiles_total) >= min_coverage:
                break
            
            #> Otherwise, let each robot clean its tile and recount the number of cleaned tiles
            else:
                for robot in robots:
        ...
        time_steps_total.append(time_steps)
    
    return sum(time_steps_total) / len(time_steps_total)
...
#       of a 20x20 room?
#       
#       Overall, the StandardRobot takes fewer steps than the FaultyRobot for all number of robots.
#       For both robot types, the time it takes to clean decreased exponentially as the number of robots increases.
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
#
#       Overall, the StandardRobot takes fewer steps than the FaultyRobot for all room shapes.
#       For the FaultyRobot, the time decreases as the aspect ratio decreases until around 1.25, then increases quickly as the aspect ratio increases.
#       For the StandardRobot, the time decreases as the aspect ratio decreases until around 2, then increases thereafter as the aspect ratio increases.
